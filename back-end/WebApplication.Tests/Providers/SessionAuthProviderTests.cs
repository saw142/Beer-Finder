...
namespace WebApplication.Tests.Providers
{
    ...
    public class SessionAuthProviderTests
    {
        ...
        public void GetCurrentUser_Should_ReturnsUserIfLoggedIn()
        {
            ...
            AddUserToDAL("test", "test@example.com");
            AddUserToSession("test");
        ...
        public void UserExists_Should_CheckIfUserAlreadyExists()
        {
            ...
            // Act
            var trueResult = provider.UserExistsCheck("test", "test@example.com");
            var falseResult = provider.UserExistsCheck("test1234", "test@example.com");
        ...
        public void Register_Should_SaveUserToDatabase()
        {
            ...
            // Act
            provider.Register("test","test@example.com", "password123", "user");
        ...
        private void AddUserToDAL(string username, string email)
        {
            var user = new User { Username = username, Email = email, Password = "OZpnzNCj1mcK3lvPKxhh89ikT0w=", Salt = "rsD3TaOu0XQ=" };
            mockUserDal.Setup(m => m.GetUser(username)).Returns(user);
            mockUserDal.Setup(m => m.UserExists(username, email)).Returns(true);
        }
